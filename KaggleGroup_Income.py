...
x_mod['Gender'].fillna("missing", inplace = True)
x_mod['Year of Record'].fillna(1990, inplace = True) #using a string was causing some problems
x_mod['Satisfation with employer'].fillna("missing", inplace = True)
x_mod['Country'].fillna("missing", inplace = True)
x_mod['University Degree'].fillna("missing", inplace = True)
x_mod['Hair Color'].fillna("missing", inplace = True)
x_mod['Profession'].fillna("missing", inplace = True)

#Dealing with unwanted values
x_mod['Gender'] = x_mod['Gender'].replace('0', 'missing')
x_mod['Gender'] = x_mod['Gender'].replace('unknown', 'missing')
x_mod['Hair Color'] = x_mod['Hair Color'].replace('0', 'missing')
x_mod['Hair Color'] = x_mod['Hair Color'].replace('Unknown', 'missing')
x_mod['University Degree'] = x_mod['University Degree'].replace('0', 'missing')
...
x_test['Year of Record'].fillna(1990, inplace = True)
x_test['Size of City'].fillna("missing", inplace = True)
x_test['Yearly Income in addition to Salary (e.g. Rental Income)'].fillna("missing", inplace = True)
#x_test['Work Experience in Current Job [years]'].fillna(method = 'ffill', inplace = True)
x_test['Satisfation with employer'].fillna("missing", inplace = True)
x_test['Gender'].fillna("missing", inplace = True)
x_test['Country'].fillna("missing", inplace = True)
x_test['University Degree'].fillna("missing", inplace = True)
x_test['Hair Color'].fillna("missing", inplace = True)
x_test['Profession'].fillna("missing", inplace = True)

#Dealing with unwanted values
x_test['Gender'] = x_test['Gender'].replace('0', 'missing')
x_test['Gender'] = x_test['Gender'].replace('unknown', 'missing')
x_test['Hair Color'] = x_test['Hair Color'].replace('0', 'missing')
x_test['Hair Color'] = x_test['Hair Color'].replace('Unknown', 'missing')
x_test['University Degree'] = x_test['University Degree'].replace('0', 'missing')
